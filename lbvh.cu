...
void BVH::draw(const Node* vertices, size_t numVerts, image_rgb8& img)
{
    thrust::host_vector<LBVHNode> nodes(impl_->nodes);
    auto bbox = nodes[0].bbox;
    vec2 img_size(img.width(), img.height());
    for (size_t i = 0; i < nodes.size(); ++i)
    {
        auto bb = nodes[i].bbox;

        bb.min -= bbox.min;
        bb.min /= bbox.max - bbox.min;
        bb.min *= img_size;

        bb.max -= bbox.min;
        bb.max /= bbox.max - bbox.min;
        bb.max *= img_size;

        draw_line(bb.min.x,bb.min.y, bb.max.x,bb.min.y, img);
        draw_line(bb.max.x,bb.min.y, bb.max.x,bb.max.y, img);
        draw_line(bb.max.x,bb.max.y, bb.min.x,bb.max.y, img);
        draw_line(bb.min.x,bb.max.y, bb.min.x,bb.min.y, img);
    }

    for (size_t i = 0; i < numVerts; ++i)
    {
        Node vert = vertices[i];
        vert -= bbox.min;
        vert /= bbox.max - bbox.min;
        vert *= img_size;

        basic_aabb<float, 2> bb(vert,vert+vec2(1));

        draw_line(bb.min.x,bb.min.y, bb.max.x,bb.min.y, img, {255,0,0});
        draw_line(bb.max.x,bb.min.y, bb.max.x,bb.max.y, img, {255,0,0});
        draw_line(bb.max.x,bb.max.y, bb.min.x,bb.max.y, img, {255,0,0});
        draw_line(bb.min.x,bb.max.y, bb.min.x,bb.min.y, img, {255,0,0});
    }
}

...
struct Layouter::Impl
{
    ...
    Impl(Graph& g, image_rgb8& img, std::mutex& mtx, LayouterMode mode, unsigned rebuildRTXAccelAfter)
        : graph(g)
        , img(img)
        , mtx(mtx)
        , mode(mode)
        ...
        , d_edges(g.edges.size())
    {
	    ...
	    std::cout << "LayouterMode: RTX" << std::endl;
            ...
            launchParams = owlLaunchParamsCreate(context,sizeof(GraphGeom),
                                                 launchParamsVars, -1);
            ...
            owlLaunchParamsSetBuffer(launchParams, "nodes", rtx_nodes);
            owlLaunchParamsSet1ui(launchParams, "numNodes", graph.nodes.size());
            owlLaunchParamsSet1f(launchParams, "k", k);
            ...
            owlLaunchParamsSetGroup(launchParams, "world", tlasGroup);
            ...
            rtx_disp = owlDeviceBufferCreate(context,
                                             ...
                                             graph.nodes.size(),
                                             nullptr);
            owlLaunchParamsSetBuffer(launchParams, "disp", rtx_disp);
    ...
    void goRTX(double &buildTime, cuda::timer &t)
    {
        ...
        //Traverse
        owlParamsLaunch2D(raygenProg,graph.nodes.size(),1,launchParams);
    }
    ...
    void drawLBVH()
    {
        ...
        lbvh.draw(graph.nodes.data(), graph.nodes.size(), img);
        save_image("lbvh.pnm",img);
    }
...
Layouter::Layouter(Graph& g, image_rgb8& img, std::mutex& mtx, LayouterMode mode, unsigned rebuildRTXAccelAfter)
    : impl(new Impl(g, img, mtx, mode, rebuildRTXAccelAfter))
{
}
...
#if 0
    thrust::host_vector<vec2> h_disp(impl->graph.nodes.size());
    cudaMemcpy(h_disp.data(),
               dispPointer,
               sizeof(vec2)*h_disp.size(),
